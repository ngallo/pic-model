...
compiler and provides \texttt{(SYSX)BILL}, the system billing file, as the name of the
debugging output. The compiler opens the target for output using its own authority over
\texttt{SYSX}, thereby overwriting \texttt{(SYSX)BILL}. The user never possessed this
privilege, but the deputy exercised it on behalf of the user. The failure arises from
ambient authority and lack of provenance.
...
analogous to Hardy's original example.

Further assume:

\begin{quote}
The payload of a request (e.g., an output file name supplied by the user)
may influence control flow in $COMP$, including code paths that open a
resource $r$ for output using $COMP$'s own authority.
\end{quote}
...
\begin{proof}
Since $(\text{write},r)\in Priv(COMP)$ (H2), an internal code path in $COMP$
may open $r$ for output using its own ambient authority.
Because request processing is influenced by user-supplied parameters
(such as the target file name), there exists an adversarial payload $req^\*$
that causes $COMP$ to select $r$ and execute $(\text{write},r)$.
By (H1), the user $U$ does not possess $(\text{write},r)$, yet by sending $req^\*$
to the deputy $COMP$, $U$ causes $COMP$ to exercise this privilege on $U$'s behalf.
All conditions of the Confused Deputy definition are therefore satisfied.
\end{proof}

This applies to all artifact-based delegation models:
sealing or encryption protects transport, not authority semantics
(e.g., JWT bearer tokens, sealed capabilities, cryptographic credential sets).
...
\begin{lemma}[Early rejection]
Let $\pi = \langle (p_0,ops_0),\dots,(p_j,ops_j),\dots,(p_n,ops_n)\rangle$
be a monotonic chain, i.e.\ $ops_{i+1}\subseteq ops_i$ for all $i$.
If $(o,r)\notin ops_j$ for some $0\le j\le n$, then 
$(o,r)\notin ops_k$ for all $k \ge j$.
\end{lemma}

\begin{proof}
By monotonicity, $ops_k \subseteq ops_j$ for all $k\ge j$.
Since $(o,r)\notin ops_j$, it cannot belong to any of its subsets.
\end{proof}

\paragraph{Interpretation.}
Loss of authority is irreversible: if a privilege is absent at hop $j$,
no subsequent hop can recover it. Therefore a compliant system may reject
at hop $j$ without waiting for the final authorization stage.
...
then $(\text{write},r)$ can never be authorized at hop $n$.
\end{theorem}

\begin{proof}
Authorization requires:
\[
(\text{write},r) \in \bigcap_{i=0}^{n} ops_i.
\]
But $(\text{write},r)\notin ops_0$ and each $ops_{i+1}\subseteq ops_i$.
Therefore $(\text{write},r)\notin ops_i$ for all $i$,
