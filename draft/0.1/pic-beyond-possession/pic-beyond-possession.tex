...
We make that distinction comparable across mechanism families by representing
an authorization model as an observation of a common occurrence domain
together with its transition invariants. Kernel inclusion then defines a
partial order on observation-kernel classes through which concrete
realizations can be compared, not a ranking of security.
...
permitted to exercise it in the current causal history. Bare possession does
not determine \(\operatorname{Lineage}(e)\) or
\(\PoR(s_i,s_{i+1})\).
...
P(s)=P(s').
\]
Thus a lineage-invariant or artifact-local policy cannot distinguish two
otherwise identical privilege uses solely by their different causal histories.
A capability construction that verifies an equivalent lineage-sensitive
continuation condition lies outside this invariant policy class; the
factorization result does not claim that every capability system is
lineage-invariant.
...
\begin{enumerate}
    ...
    \(\tau_i=(q_J(x_i),q_J(x_{i+1}))\) and checks \(\rho_i(\tau_i)\); and
...
\boxed{
\begin{gathered}
\text{causal evidence accumulates while}\\
...
\]
The result is internal to the PIC semantics and uses neither
\(\operatorname{Priv}(\exec(s_i))\), Complete Mediation, nor Causal
Attribution.
...
\]
whereas the relational rule evaluates an occurrence through
...
For an authorization model \(M\), an \emph{observation map} is a surjection
\[
...
\]
A Boolean admission policy based on this observation has the form
\[
P_M=\bar P_M\circ\operatorname{Obs}_M
\]
for some \(\bar P_M:Y_M\to\{0,1\}\).
\end{definition}
...
of Remark~\ref{rem:instance}. Both the explicit history coordinate and the
evidence coordinates from which that history could be recovered are now
forgotten by construction. It does not say that any named protocol family is
necessarily lineage-forgetting: a realization can retain sound causal-history
facts in its observation.

Observation is only one axis. Let
\[
\operatorname{Trans}_M\subseteq\Omega\times\Omega
\]
denote the transitions that model \(M\) admits. We therefore classify a model
by the pair
\[
\mathfrak M=(\operatorname{Obs}_M,\operatorname{Trans}_M).
\]
For
\[
\begin{aligned}
...
\begin{itemize}
    ...
    request-binding hook, but PIC-X's artifact-linked policy explicitly accepts
    this check because no trusted request or transport occurrence is available
    at that boundary. A checkpoint is therefore not tied to the request it
    continues.
...
not define semantic classes. A family is characterized by the kernel of the
observation actually used for admission and by its admitted transition
relation. No protocol family below is asserted to be a set-theoretic subset of
PIC. The rows summarize mechanism profiles; they do not assign one canonical
...
\addlinespace
Attenuating capability or delegation artifact
& Artifact, restrictions and artifact ancestry exposed by the construction
& Construction-dependent
& Enforced when descendants can only preserve or restrict authority
& Construction-dependent; artifact ancestry may or may not witness occurrence continuity \\
\addlinespace
PIC-compatible authorization
& Sound occurrence, causal-history and executor-profile facts used by the admission rule
...
by all uses of OAuth, JWT or SAML. If that observation factors through
\(\pi_{\neg L}\), Corollary~\ref{cor:lineage-forgetting} applies. The
standards remain free to carry or reference additional evidence; a profile that
soundly observes causal occurrence belongs to a finer or different class.
...
therefore refines possession equivalence without necessarily recovering
causal-history equivalence. A concrete profile may nevertheless add sound
continuity evidence and, if it enforces the required transition obligations,
become a PIC-compatible realization.

\paragraph{Attenuating capability and delegation artifacts.}
...
not place every construction in one observation class. An implementation that
soundly identifies artifact ancestry with occurrence continuity and enforces
all four obligations of Definition~\ref{def:pic-compatible} is a
capability-based PIC-compatible realization. PIC does not replace its carrier
or policy language.
...
complementary: each can supply observations or checks, while the pair
\((\operatorname{Obs}_M,\operatorname{Trans}_M)\) records which distinctions
and transition invariants the resulting system actually uses.
...
observation map determines which occurrences a policy can distinguish, while a
separate transition relation determines which state evolutions it admits.
...
Proposition~\ref{prop:mechanism-invariance}, any executor with the profile of \(x_2\), for
