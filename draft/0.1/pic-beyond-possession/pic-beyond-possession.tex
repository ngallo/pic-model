...
(o,r)\notin C_0
\Longrightarrow
(o,r)\text{ cannot be a valid in-lineage exercise}.
}
\]
...
\text{PIC validity}
\Longrightarrow
\text{no in-lineage confused-deputy authority mixing}.
}
\]
...
\(\lambda\), with
\[
\operatorname{stepCommitment}(\kappa_i)=h(s_i),
\qquad
\operatorname{stepCommitment}(\kappa_{i+1})=h(s_{i+1}).
\]
...
\text{PoR establishes causal continuation;}\\
\text{authorization combines PoR, the profiles, \(C_i\), and \(\rho_i\)}\\
\text{to decide what the occurrence may exercise in that history.}
\end{array}
}
\]
...
\boxed{
\begin{array}{c}
\text{possessing authority}\neq\text{being the occurrence permitted}\\
\text{to exercise it in this causal history}
\end{array}
}.
\]
