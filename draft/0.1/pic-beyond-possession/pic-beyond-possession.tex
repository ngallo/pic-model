...
Even if \(\exec(s_i)=D\) and
\(\operatorname{IndependentAuthority}(D,(o,r))\) holds, possession does not
inject \((o,r)\) into \(C_i\). The model neither identifies \(C_i\) with
\(\operatorname{Priv}(\exec(s_i))\) nor supplies an update rule
\[
C_i\leftarrow C_{i-1}\cup\operatorname{Priv}(\exec(s_i)).
\]
Such an update would be an authority expansion unless the added privileges
were already in \(C_{i-1}\), whereas validity requires
...
\end{aligned}
\]
The consumption-register check for \(\kappa_i\) must also have succeeded when
\(\kappa_{i+1}\) was issued. This
...
corresponding base bindings and sound origin payload. By protocol compliance,
