...
lineage-invariant possession-based decision cannot distinguish occurrences
that differ only in causal history (Remark~\ref{rem:instance}).
...
Capability-, caveat-, and logic-based authorization systems can encode or
consume contextual and history-sensitive facts, including expiry, lineage or
predecessor identifiers, delegation ancestry, ambient facts, and externally
attested continuation evidence. The distinction claimed here is therefore not
raw policy-language expressiveness. This model makes causal history a primitive
dimension of the authorization domain and proves authority and relational
closure along authorization-aware histories.

A mechanism does not already have PIC semantics merely because it can encode
the facts that PIC requires. It is a PIC-compatible realization only when those
facts are soundly established and the corresponding causal-history,
authority-attenuation, relational-admissibility, and refinement obligations
are enforced at admission. Representability of PIC premises is therefore not
equivalent to PIC semantics.

An artifact-centered system can reproduce the same decision when supplied with
equivalent trusted causal facts. Doing so requires more than adding a lineage
field: at each hop it must soundly establish an equivalent valid continuation
and the profile facts used in \(\tau_i\), bind the state to the corresponding
history, check \(C_{i+1}\subseteq C_i\), evaluate \(\rho_i(\tau_i)\), and check
\(\rho_{i+1}\Rightarrow\rho_i\). These are exactly the obligations of
Definition~\ref{def:pic-compatible}. Bare artifact possession supplies none of
them by itself. Both such realizations and the witness construction require
trusted evidence for causal facts; the witness assumption
\(\operatorname{ObservedHandoff}\Rightarrow\PoR\) does not create causal truth.
The single-consumption/anti-replay rule of Section~\ref{sec:witness} supports
that construction's additional claim that a checkpoint has at most one
accepted successor; it is not an extra conjunct of the core transition
semantics.

A logic-based artifact can therefore consume an externally established
\texttt{valid\_continuation} fact. The remaining questions are what semantics
makes that fact a sound witness of \(\PoR\), and whether admission enforces the
two monotone obligations and relational admissibility. When those conditions
hold, the artifact remains a capability carrier or policy engine, while the
resulting authorization system is a capability-based PIC-compatible
realization. This is a claim about the semantic structure of admission, not
about greater raw expressiveness of one policy language over another.
...
An extended capability system that verifies a lineage-sensitive continuation
condition can therefore enforce the same in-lineage non-mixing property. Claims
about request-caused effects outside the represented lineage still require an
equivalent of Complete Mediation and Causal Attribution; encoding the check in
a capability does not supply those premises by itself.
Capability mechanisms therefore remain valid as representations and carriers
of authority inside the broader causal-history authorization model.
...
causal history, not a holder or artifact alone. PIC does not replace capability
mechanisms as representations or carriers of authority; it changes the
semantic space in which carried authority may be exercised. Capabilities carry
authority; PIC additionally governs whether this execution occurrence, in this
causal history, may exercise that authority. Evidence used to establish PoR is
not the semantic relationship itself, just as evidence used to establish a
profile is not the authorization decision.

Under Definition~\ref{def:pic-compatible}, systems built from capabilities,
Macaroons, Biscuit, UCAN or similar artifacts are PIC-compatible when they
soundly establish equivalent causal-continuation and profile facts and enforce
the same monotone lineage obligations. The artifact remains their authority
carrier or policy engine; the semantics governing its exercise is
PIC-compatible. This is why such a construction is a realization of the
broader causal-history authorization semantics rather than a counterexample to
it.

PIC therefore accommodates capabilities, credentials, caveats and PoP as
evidence mechanisms without reducing authorization to their possession. It
changes the domain in which their evidence is consumed: authorization applies
to execution occurrences situated in causal histories.
Consequently,
\[
\boxed{
\begin{array}{c}
