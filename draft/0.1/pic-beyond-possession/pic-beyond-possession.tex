...
Authorization is occurrence-sensitive: a transition is evaluated from PoR,
non-expansion of its authority context, satisfaction and refinement of its
relational envelope, and soundly established executor profiles.
...
As an intuition rather than a definition, PoP answers which artifact a
presenter demonstrates control over; PoR answers which execution occurrence is
being continued; and authorization answers what that occurrence may exercise
in its causal history. PoR is not by itself the authorization decision, and a
PoP transcript is not by itself a relational profile or a causal history.
...
field: at each hop it must establish an equivalent valid continuation, satisfy
the relevant evidence-soundness obligations, bind the predecessor and history,
enforce the protocol's replay rule, check \(C_{i+1}\subseteq C_i\), check
\(\rho_{i+1}\Rightarrow\rho_i\), and evaluate \(\rho_i(\tau_i)\). Bare
artifact possession supplies none of those facts by itself. Both such
realizations and the witness construction require trusted evidence for causal
facts; the witness assumption
...
Evidence used to establish PoR is not the semantic relationship itself, just as
evidence used to establish a profile is not the authorization decision.
It does not eliminate capabilities, credentials, caveats or PoP as evidence
mechanisms. It changes the domain in which their evidence is consumed:
authorization applies to execution occurrences situated in causal histories.
Consequently,
\[
\boxed{
\begin{array}{c}
