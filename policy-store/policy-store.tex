...
Note that the unit is the policy, not the file. A single source file may
well contain several policies: each of them becomes a distinct object,
with its own identifier and its own properties. The file exists only to
make authoring easier; once the policies are turned into objects, it
...
resource $R$ in context $C$?} It is a reasonable shape, and it has served
well: it captures application policies faithfully, and it is where the
industry has rightly concentrated its effort.

Once executions are looked at end to end, however, questions emerge that
this shape was never designed to answer. Execution policies do not speak
about subjects and resources: they speak about steps, paths, and flows.
Translation policies do not answer an access question at all: they map
authority between vocabularies. If a translation policy must be evaluated
when the execution crosses a domain boundary, what evaluates it --- a
second, ad hoc decision point? If an execution policy must be validated
inside the network path, does the network now host an application-style
PDP?

These are not shortcomings of any particular design: they are problems we
all share, surfaced by distributed execution and autonomous agents. The
observation they lead to is simple: both the store and the evaluation
model benefit from being aware of policy types, rather than assuming that
the application type is the whole picture.
...
\]
The symbol $\cap$ simply reads ``what appears in both'': a privilege is
effective only if the policies grant it \emph{and} the authority received
at entry already contained it. Anything a policy grants beyond the entry
authority is clipped; without this rule, authority expands.

This is zero trust taken to its full consequence. Zero trust is commonly
read as \emph{never trust, always verify}, applied to identities and
connections; applied to authority, it means that every boundary is a
point of \term{attenuation}, never of amplification. Segmentation at the
execution level narrows where the transaction may flow; translation at
the domain level maps authority without enlarging it; evaluation at the
application level grants at most what the origin already allowed.
Attenuation applies at every level --- execution, domain, application ---
and the intersection rule is what makes it hold end to end.
...
This grouping has a consequence on the evaluation side as well. If a
group declares partitions for different policy types, the decision
point's interface must be able to address them: a caller needs a way to
state which partition --- which policy type --- a request targets.
Evaluation interfaces designed around the application type alone are the
natural starting point, but for policy types to work end to end they need
to be extended in this direction, so that the manifest and the interface
...
no query engine, no storage-specific format. The same property scales
down as well as up: a low-resource device can keep its objects in a small
key-value store, and even a browser can persist them in its local
storage. This keeps the model deployable everywhere the policies need to
go --- from cloud services to edge nodes, constrained devices, and
...
Adopting Git as-is, however, would carry more than the problem needs. It
brings along working trees, branching and merging workflows, and hooks;
as a general-purpose content tracker it enforces no structure --- anything
can be committed, and nothing guarantees the invariants of a policy group
(a manifest, partitions, one policy per object); and it was not designed
for the evaluation path, where a decision point must materialize a
precise version of a group quickly, at every decision. A more fitting
approach is a \term{git-like} object model, specialized for policies:
battle-tested ideas, without reinventing the wheel and without inheriting
what does not serve. We call the resulting structure a \term{ledger}.
...
\subsection{Negotiated Synchronization}

Synchronization needs one more ingredient: a \term{negotiation} protocol.
A consumer is not a passive receiver of whatever the store pushes: it
must be able to state what it needs, and the two sides must agree on the
smallest transfer that satisfies it. The negotiation spans a few natural
axes:

\begin{itemize}[nosep]
  \item \textbf{Scope} --- everything, a single group, or only selected
        partitions of a group: a decision point serving application
        decisions may have no use for the execution partitions, and vice
        versa;
  \item \textbf{Version} --- the current head, a specific commit, or a
        bounded slice of history;
  \item \textbf{Capability} --- only the partitions whose declared
        language and runtime the consumer actually supports, as read from
        the manifest;
  \item \textbf{Delta} --- given the last commit the consumer already
        holds, only the objects it is missing.
\end{itemize}

The mechanics follow from the ledger itself. The two sides exchange refs
--- each declares the commit it holds --- and walk the predecessor chain
to compute what is missing; content addressing makes ``already present''
verifiable, so nothing is transferred twice. A sketch of the dialogue:

\begin{lstlisting}
consumer: group "commerce-authz": I hold commit #C1;
          I support cedar >= 4.0; I serve partitions "/" and "/agents"
store:    head is #C3; between #C1 and #C3 you are missing #C2, #C3;
          restricted to your partitions, that is 7 objects
consumer: transfer them
\end{lstlisting}

A consumer with abundant resources can mirror the entire store; a
constrained one can keep a razor-thin replica --- exactly the groups,
partitions, and versions it evaluates --- kept aligned by the same
protocol.

\subsection{A Runtime, Not a Bundle}

A natural first answer to distribution is a compressed archive: package
the files, ship the bundle to the decision point, unpack, load. Archives
are simple and familiar, and for occasional deployments they can be
enough. They give up, however, most of what the object model provides:
filesystem semantics return in place of self-describing objects, the
whole store travels instead of a delta, and there is no per-object
verification, no resumable exchange, no history at the destination.
Policies end up treated as static configuration to be installed rather
than as governed, versioned objects.
